...
class KronosPredictor:
    ...
    def predict(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True):
        ...
        if not all(col in df.columns for col in self.price_cols):
            raise ValueError(f"价格列 {self.price_cols} 未在 DataFrame 中找到。")
        ...
        if df[self.price_cols + [self.vol_col, self.amt_vol]].isnull().values.any():
            raise ValueError("输入 DataFrame 中包含价格或成交量列的 NaN 值。")
    ...
    def predict_batch(self, df_list, x_timestamp_list, y_timestamp_list, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True):
        ...
        if not isinstance(df_list, (list, tuple)) or not isinstance(x_timestamp_list, (list, tuple)) or not isinstance(y_timestamp_list, (list, tuple)):
            raise ValueError("df_list, x_timestamp_list, y_timestamp_list 必须是列表或元组类型。")
        if not (len(df_list) == len(x_timestamp_list) == len(y_timestamp_list)):
            raise ValueError("df_list, x_timestamp_list, y_timestamp_list 必须具有一致的长度。")
        ...
        for i in range(num_series):
            df = df_list[i]
            if not isinstance(df, pd.DataFrame):
                raise ValueError(f"索引 {i} 处的输入不是 pandas DataFrame 类型。")
            if not all(col in df.columns for col in self.price_cols):
                raise ValueError(f"索引 {i} 处的 DataFrame 缺少价格列 {self.price_cols}。")
            ...
            if df[self.price_cols + [self.vol_col, self.amt_vol]].isnull().values.any():
                raise ValueError(f"索引 {i} 处的 DataFrame 包含价格或成交量列的 NaN 值。")
            ...
            if x.shape[0] != x_stamp.shape[0]:
                raise ValueError(f"索引 {i} 处的 x 与 x_stamp 长度不一致：x 有 {x.shape[0]} 行，而 x_stamp 有 {x_stamp.shape[0]} 行。")
            if y_stamp.shape[0] != pred_len:
                raise ValueError(f"索引 {i} 处的 y_timestamp 长度应为 pred_len={pred_len}，但 got {y_stamp.shape[0]}。")
        ...
        if len(set(seq_lens)) != 1:
            raise ValueError(f"索引 {i} 处的并行预测要求所有系列都具有一致的历史长度，got: {seq_lens}")
        if len(set(y_lens)) != 1:
            raise ValueError(f"索引 {i} 处的并行预测要求所有系列都具有一致的预测长度，got: {y_lens}")
